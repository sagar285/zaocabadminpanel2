...
import { useUploadAadharCardMutation, useUploadDriverLicenseMutation } from '../../Redux/Api';

const AddDrivingLicenseDocumentModal = ({ isOpen, onClose, docType, userId,refetch }) => {
  const [formData, setFormData] = useState({
    frontImage: null,
    backImage: null,
    number: ''
  });
  const [uploadDriverLicense] =useUploadDriverLicenseMutation();
  ...
  const handleSubmit = async(e) => {
    ...
    if (formData.frontImage) data.append('license_front', formData.frontImage);
    if (formData.backImage) data.append('license_back', formData.backImage);
    if (formData.number) data.append('licenseNumber', formData.number);
    data.append("userId",userId);
    data.append("isAdmin",true);
    const response = await uploadDriverLicense(data);
      ...
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-blue-500 transition-colors">
        {preview ? (
          ...
          <label className="flex flex-col items-center justify-center h-48 cursor-pointer">
            ...
            <span className="text-sm text-gray-500">Click to upload {side === 'frontImage' ? 'front' : 'back'} image</span>
          </label>
        )}
      </div>
    </div>
...
export default AddDrivingLicenseDocumentModal;